...
<%
	...
	c.setNome("");
	c.setEndereco("");
	List<Cliente> clientes = new ArrayList<Cliente>(c.buscarClientes());
	...
	if (request.getParameter("del") != null) {
		c.deletarCliente(Long.valueOf(request.getParameter("id")));
		response.sendRedirect(request.getRequestURI());
	}
	if (request.getParameter("alt") != null) {
		c = clientes.get(Integer.parseInt(request.getParameter("i")) - 1);
	}
				...
				<form>
					...
					<div class="form-group">
						<label for="log-end">Endereço:</label> <input id="log-end"
							...
							class="form-control" value="<%=c.getEndereco()%>">
					</div>
					<%
						if (request.getParameter("alt") != null) {
					%>
